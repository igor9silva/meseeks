---
title: shadcn-like component libraries
tags: [tech, ux, source:ticktick, ticktick-list:references, ticktick-list:meseeks, ticktick-status:user-interface, class:reference]
---

Searchable reference collection for shadcn-like components, AI UI components, blocks, themes, and patterns we may want to reuse or study.

- https://21st.dev/ - collection
- https://retroui.dev
- https://reactflow.dev/components
- https://www.simple-ai.dev
- https://originui.com
- https://www.ui-layouts.com/components
- https://www.fancycomponents.dev/docs/components/text/variable-font-and-cursor
- https://shadcn-country-dropdown.vercel.app/
- https://magicui.design
- https://ui.indie-starter.dev/form-builder
- https://shadcnui-expansions.typeart.cc
- https://motion-primitives.com
- https://www.shadcn-form.com
- https://www.prompt-kit.com
- https://ui.spectrumhq.in
- https://www.kibo-ui.com/
- https://shadcnblocks.com
- https://shadcnstudio.io
- https://coss.com/ui
- https://reui.io
- https://reactbits.dev
- https://tremor.so
- https://midday.ai/components
- https://elements.ai-sdk.dev/components/agent — AI agent/tool-schema UI reference.
- https://ai-sdk.dev/elements/components/agent
- https://x.com/haydenbleasel/status/2014035526324723816?s=12
- [ElevenLabs live waveform](../elevenlabs-live-waveform/_index.md) — audio UI components and realtime waveform reference.
