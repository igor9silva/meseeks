...
const SOURCE_WORKSPACE_FILE = join(PROJECT_ROOT, 'meseeks.code-workspace');
const GENERATED_WORKSPACE_FILE = join(
	PROJECT_ROOT,
	'meseeks.codex.generated.code-workspace',
);
...
const GENERATED_HEADER =
	'// Generated by .config/sync-codex-worktrees.ts. Edit meseeks.code-workspace instead.\n';
...
function main(): void {
	...
	watchers.push(watch(SOURCE_WORKSPACE_FILE, requestSync));
...
function syncWorkspaceFile(): SyncResult {
	//
	const sourceContent = readFileSync(SOURCE_WORKSPACE_FILE, 'utf8');
	const sourceFolders = readWorkspaceFolders(sourceContent);
	const sourceDirectory = dirname(SOURCE_WORKSPACE_FILE);
	const outputDirectory = dirname(GENERATED_WORKSPACE_FILE);
	const baseFolders = sourceFolders.filter(
		(folder) => !isManagedCodexWorktreeFolder(folder, sourceDirectory),
	);
	const baseFolderPaths = new Set(
		baseFolders.map((folder) => normalizeFolderPath(folder.path, sourceDirectory)),
	);
	...
	const nextContent = `${GENERATED_HEADER}${replaceWorkspaceFolders(
		sourceContent,
		nextFolders,
	)}`;
	const previousContent = existsSync(GENERATED_WORKSPACE_FILE)
		? readFileSync(GENERATED_WORKSPACE_FILE, 'utf8')
		: '';
	const changed = previousContent !== nextContent;

	if (changed && !DRY_RUN) {
		mkdirSync(outputDirectory, { recursive: true });
		writeFileAtomic(GENERATED_WORKSPACE_FILE, nextContent);
	}

	const previousFolders = previousContent.startsWith(GENERATED_HEADER)
		? readWorkspaceFolders(previousContent.slice(GENERATED_HEADER.length))
		: [];
	const previousCodexFolders = previousFolders.filter((folder) =>
		isManagedCodexWorktreeFolder(folder, outputDirectory),
	);
	...
	const previousCodexFolderPaths = new Set(
		previousCodexFolders.map((folder) =>
			normalizeFolderPath(folder.path, outputDirectory),
		),
	);
	...
	return {
		...
		changed,
		outputPath: GENERATED_WORKSPACE_FILE,
		removed: previousCodexFolders.filter(
			(folder) =>
				!nextCodexFolderPaths.has(normalizeFolderPath(folder.path, outputDirectory)),
		),
