...
- Treat `tasks/TAGS.md` as executable cleanup guidance. Apply its bucket, visibility, and canonical-task constraints directly when the move is mechanical and safe.
- Do not apply current tag cleanup to `completed/`. Completed tasks are history; leave their paths, visibility, tags, and bodies alone unless the user explicitly asks.
