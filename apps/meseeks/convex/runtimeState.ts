...
export const _upsertDetails = internalMutation({
	...
	handler: async (ctx, { owner, actionId, ...value }) => {
		...
		return await ctx.db.insert('action_details', {
			action: actionId,
			costs: value.costs ?? [],
			...patch,
			createdAt: now,
...
export const _settle = internalMutation({
	...
	handler: async (ctx, { owner, ...args }) => {
		...
		const planPatch = await applyPlanMutation(ctx, {
			owner,
			actionId: args.actionId,
			result: args.result,
		});
		const iterationPatch = await applyIterationMutation(ctx, {
			owner,
			actionId: args.actionId,
			result: args.result,
		});
		const settled = await settleAction(ctx, {
			...args,
			patch: combinePatches(args.patch, planPatch, iterationPatch),
			shouldReleaseAvailableBudget: shouldReleaseAvailableBudgetOnSettle(args.result),
		});
		const statePatch = await applySettledResultState(ctx, {
			owner,
			actionId: args.actionId,
			result: args.result,
		});
		if (!statePatch) return settled;

		const actionAfterState = await ctx.db.get(args.actionId);
		if (!actionAfterState || actionAfterState.owner !== owner) throw NotFound();
		const patch = combinePatches(actionAfterState.patch, statePatch);
...
function combinePatches(...values: Array<string | undefined>) {
	//
...
async function applySettledResultState(
	...
	const tagPatch = await setFileTags(ctx, {
		owner: args.owner,
		file: action.file,
		author: args.actionId,
