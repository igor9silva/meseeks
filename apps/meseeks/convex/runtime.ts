...
const managedThinkSkillNames = ['think', 'plan', 'iterate'];
...
async function performClaimedAction(args: {
	...
	if (skillKind === 'think' || isManagedThinkSkill(skillName)) {
		return await performThinkSkill({
			context: parsedContext,
			maxCost: args.maxCost,
		});
	}

	if (skillKind === 'execute' || skillName === 'execute') {
...
const runtimeContextSchema = z.object({
	...
	skill: z
		.object({
			...
			kind: z.enum(['instinct', 'think', 'request', 'execute']),
...
function isManagedThinkSkill(skill: string) {
	//
...
function usesRuntimeIntelligence(context: z.infer<typeof runtimeContextSchema>) {
	//
	return context.skill?.kind === 'think' || isManagedThinkSkill(context.action.skillKey);
}
...
async function performThinkSkill(args: { context: z.infer<typeof runtimeContextSchema>; maxCost: bigint }) {
	//
	const resolvedIntelligence = resolveRuntimeIntelligence({
		context: args.context,
	});
	...
	const instructions = thinkSkillInstructions(args.context);
	if (!instructions) {
		return {
			status: settledActionStatusSchema.parse('failed'),
			result: {
...
function structuredOutputSettings(args: {
	...
	//
	if (!isStructuredThinkSkill(args.skill)) return {};
...
function isStructuredThinkSkill(skill: string) {
	//
...
function maxOutputTokensForSkill(skill: string) {
	//
	if (isStructuredThinkSkill(skill)) return 4096;
...
function thinkSkillInstructions(context: z.infer<typeof runtimeContextSchema>) {
	//
...
function visibleThinkSkillText(args: {
	skill: string;
	text: string;
...
function estimateClaimCost(args: {
	...
	if (skillKind !== 'think' && args.context.action.skillKey !== 'think') {
		return {
			expectedCost: 0n,
			maxCost: 0n,
		};
