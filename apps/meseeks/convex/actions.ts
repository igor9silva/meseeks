...
import { claimEndpoint, listEndpoints, unclaimEndpoint } from './endpoints.private';
...
const claimEndpointSkillArgsSchema = z.object({
	file: zid('files').optional(),
	handler: zid('files'),
});
const unclaimEndpointSkillArgsSchema = z.object({
	endpointId: zid('endpoints'),
});
const listEndpointsSkillArgsSchema = z.object({
	file: zid('files').optional(),
});
...
async function createActionForSkillRequest(
				...
				files: [
					{
						...
						readMethods: ['read', 'cat', 'head', 'tail'],
					},
				],
			},
			patch: `+ file ${parsed.data.name}`,
		});
	}

	if (args.skill.skillKey === 'changeEnergy') {
	...
	if (args.skill.skillKey === 'claimEndpoint') {
		const parsed = claimEndpointSkillArgsSchema.safeParse(args.skill.args);
		if (!parsed.success) {
			return await recordFailedSkillAction(ctx, {
				owner: args.owner,
				file: args.file,
				...
				skillKey: args.skill.skillKey,
				actionArgs: args.skill.args,
				message: 'claimEndpoint arguments are invalid.',
			});
		}

		const claimed = await claimEndpoint(ctx, {
			owner: args.owner,
			file: parsed.data.file ?? args.file,
			handler: parsed.data.handler,
			author: args.owner,
	...
	if (args.skill.skillKey === 'unclaimEndpoint') {
		const parsed = unclaimEndpointSkillArgsSchema.safeParse(args.skill.args);
		if (!parsed.success) {
			return await recordFailedSkillAction(ctx, {
				owner: args.owner,
				file: args.file,
				...
				skillKey: args.skill.skillKey,
				actionArgs: args.skill.args,
				message: 'unclaimEndpoint arguments are invalid.',
			});
		}
	...
	if (args.skill.skillKey === 'listEndpoints') {
		const parsed = listEndpointsSkillArgsSchema.safeParse(args.skill.args);
		if (!parsed.success) {
			return await recordFailedSkillAction(ctx, {
				owner: args.owner,
				file: args.file,
				...
				skillKey: args.skill.skillKey,
				actionArgs: args.skill.args,
				message: 'listEndpoints arguments are invalid.',
			});
		}
	...
	if (args.skill.skillKey === 'write') {
		const content = stringArg(args.skill.args, 'content');
		if (content === undefined) return undefined;

		const patch = await writeFileContent(ctx, {
			fileId: args.file,
			owner: args.owner,
			author: args.owner,
			content,
	...
	if (args.skill.skillKey === 'rename') {
		const name = stringArg(args.skill.args, 'name');
		if (!name) return undefined;

		const patch = await moveFile(ctx, {
			fileId: args.file,
			owner: args.owner,
			author: args.owner,
	...
	if (args.skill.skillKey === 'move') {
		const parsedParent = zid('files').safeParse(args.skill.args['parent']);
		if (!parsedParent.success) return undefined;

		const patch = await moveFile(ctx, {
			fileId: args.file,
			owner: args.owner,
			author: args.owner,
			newParent: parsedParent.data,
			shouldCreateAction: false,
		});
		await recordMutationAction(ctx, {
			owner: args.owner,
	...
	if (args.skill.skillKey === 'updateFileMetadata') {
		...
		if (parsedTags.success && parsedTags.data.length > 0) {
			const patch = await setFileTags(ctx, {
				owner: args.owner,
				file: args.file,
				author: args.owner,
			...
			if (shouldReleaseBudgetForTags(parsedTags.data)) {
				const budgetPatch = await releaseAvailableFileBudget(ctx, {
					owner: args.owner,
					file: args.file,
					author: args.owner,
					actionSkill: 'updateFileMetadata',
				...
				if (budgetPatch) patches.push(budgetPatch);
			}
		}
