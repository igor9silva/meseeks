...
export const _evaluateEndpoint = internalAction({
	args: {
		owner: zid('users'),
	...
	handler: async (ctx, { owner, triggerId, file, request }): Promise<EvaluationResult> => {
		//
		const context = z
			.object({
				trigger: isolateTriggerSchema,
			})
			.parse(
				await ctx.runQuery(internal.triggerIsolateState._endpointContext, {
					owner,
					triggerId,
					file,
				}),
			);
		const proposals = await evaluateOneTriggerOrEmpty({
			trigger: context.trigger,
			context: {
				request,
				trigger: publicTriggerContext(context.trigger),
			},
		});
		const accepted = z
			.number()
