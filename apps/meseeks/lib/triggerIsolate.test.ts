...
import { endpointPathPrefix } from './endpoints';
import { endpointHandlerExamples, managedTriggerHandlers } from './proDefinitions';
...
describe('trigger isolate', () => {
	...
	test('runs endpoint handler examples as validated proposals', async () => {
		const context = {
			request: {
				method: 'POST',
				path: `${endpointPathPrefix}example`,
				headers: {
		...
		for (const code of Object.values(endpointHandlerExamples)) {
			...
			expect(result.proposals[0]?.skillKey).toBe('think');
		}
	});
