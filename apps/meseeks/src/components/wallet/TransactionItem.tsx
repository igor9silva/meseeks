...
import { ArrowUp, Clock, ExternalLink, Wallet } from 'lucide-react';
		...
		<div className="flex items-center justify-between gap-1 rounded-3xl border bg-card p-3 transition-all hover:shadow-sm">
			...
			<div className="flex flex-col items-end gap-1">
				...
				>
					<TooltipProvider>
						<Tooltip>
							<TooltipTrigger>{asDollars({ bigInt: transaction.value.amount })}</TooltipTrigger>
							<TooltipContent>
								<span className="font-semibold">
									{asDollars({ bigInt: transaction.value.amount, precision: 6 })}
								</span>{' '}
								<strong>Credits</strong>, US dollar-equivalent account funds.
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
				</span>
