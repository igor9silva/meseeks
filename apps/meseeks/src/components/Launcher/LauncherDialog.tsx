...
import { themeIconNameById } from './themes/ThemeIcon';
...
export function LauncherDialog() {
	...
	const setSearch = useCallback((nextSearch: string) => {
		setLauncherState((state) => {
			if (state.view === 'themes') return { ...state, themeSearch: nextSearch };
	...
	return (
		<CommandDialog
			...
			filter={(value, searchValue, keywords) => defaultFilter?.(value, searchValue, keywords) ?? 0}
		>
			<DialogTitle className="hidden">Launcher</DialogTitle>
			<DialogDescription className="hidden">Open product surfaces and quick actions.</DialogDescription>
			<CommandInput placeholder="Open..." value={search} onValueChange={setSearch} />
