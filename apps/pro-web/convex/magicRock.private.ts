...
import { INTELLIGENCES, type IntelligenceKey } from 'schemas/intelligenceSchema';
...
export async function prepareContext(
	...
	const maxOutputTokens = skill.config.maxTokens ?? INTELLIGENCES[intelligenceKey].context.maxOutputTokens;
...
async function renderHistory(
	...
	const history = cropHistoryToTokenLimit(
		actions
			...
			// interrupted actions finished financially, but future context should not see them
			.filter((action: Doc<'actions'>) => !('interruptedAt' in action))
