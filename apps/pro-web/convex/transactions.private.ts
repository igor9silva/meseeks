...
export const addActionReservationTransaction = defineMutation({
	args: z.object({
		value: valueSchema,
		taskId: zid('tasks'),
	...
	handler: async (ctx, args) => addTransaction(ctx, { kind: 'reserve action', ...args }),
});

export const addActionSettlementTransaction = defineMutation({
	args: z.object({
		value: valueSchema,
		taskId: zid('tasks'),
		actionId: zid('actions'),
	...
	handler: async (ctx, args) => addTransaction(ctx, { kind: 'settle action', ...args }),
});
