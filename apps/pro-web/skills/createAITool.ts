...
export function computeMaxCost(
	...
	if (!context) throw NotImplemented('Context is required to compute max cost.');
	...
	const historyLength = computeHistoryLength(context.messages);

	const inputLength = instructionsLength + toolsLength + historyLength;

	const inputTokens = Math.ceil(inputLength / env.CHAR_PER_TOKEN);
	const model = modelFrom(skill.config.model, task.preferredIntelligence);
	const outputTokens = computeMaxOutputTokens({ model, inputTokens, requestedOutputTokens: context.maxOutputTokens });
	...
	console.debug(
		`Max cost for ${skill.key} (${actionId}): ${asDollars({ bigInt: totalCost, precision: 6 })} USD`,
	);
...
function computeHistoryLength(messages: MagicRockContext['messages']) {
	//
...
function computeMaxOutputTokens({
	...
	//
	const context = INTELLIGENCES[model].context;
	const remainingContextTokens = Math.max(context.maxTokens - inputTokens, 1);
	const maxOutputTokens = requestedOutputTokens ?? context.maxOutputTokens;

	return Math.min(maxOutputTokens, remainingContextTokens);
}
