			...
			<Section title="Quality Signals" icon={<BarChart3 className="size-5" />}>
				<div className="grid gap-6 xl:grid-cols-3">
					...
					<div>
						<SectionHeading
							title="Warnings"
							detail={`${formatNumber(report.quality.warnings)} warnings outside references`}
						/>
...
function SectionFlow({ report }: { report: TaskReport }) {
	//
			...
			<div>
				...
				<div className="grid gap-3 lg:grid-cols-4">
					{lifecycleRows.map((row, index) => (
						<div key={row.label} className="relative rounded border border-border bg-card p-4">
			...
			<div>
				...
				<div className="grid gap-3 md:grid-cols-2">
					{holdingSections.map((section) => {
						...
						return (
							<div key={section} className="rounded border border-border bg-card p-4">
								<div className="flex items-center justify-between gap-3">
	...
	return (
		<div className="space-y-4">
			...
			<div>
				...
				<TaskTable rows={report.subtasks.topParents} showSection compact showWarnings={false} />
			</div>
		</div>
	);
}
...
function formatTaskSectionLabel(section: string) {
	//
...
interface TaskSort extends SortState {
	key: 'title' | 'section' | 'source' | 'priority' | 'words' | 'children' | 'warnings';
}
