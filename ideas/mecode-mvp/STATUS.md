...
- Self-contained `prototypes/mecode-mvp/` package with its own `package.json`
...
- Repository isolation: all changes are under `prototypes/mecode-mvp/`
...
- `bun run build` completes and produces a working stable `.app` bundle under `build/stable-macos-arm64/Mecode MVP.app`
...
1. Run `cd prototypes/mecode-mvp && bun install && bun run dev`
